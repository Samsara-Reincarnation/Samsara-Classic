...
script "SamsaraStrifeAccuracy" (int which)
{
    ...
    ACS_NamedExecuteAlways("SamsaraClientAccuracy", 0);
}

script "SamsaraClientAccuracy" (void) CLIENTSIDE
{
    int pln = PlayerNumber(), cpln = ConsolePlayerNumber();
    int pclass = samsaraClassNum();
    int i, j, quoteCount = 0;
    int logMsg;
    int pickupsound = ClassUniqueSounds[pclass];
...
script "SamsaraBeacon" (int noloop)
{
...
script "SamsaraSpectres" (int mode, int arg1, int arg2)
{
    int i,j,k,l, x,y,z;
    int result;
...
script "SamsaraSigil" (int baseHP)
{
